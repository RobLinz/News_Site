    ...
    <body class=homePage>
        ...
        <div id=news>
            ...
            <?php
                ...
                $stmt = $mysqli->prepare("SELECT story_ID, title, story_description, story_link FROM stories");
                ...
                $stmt->bind_result($story_id, $title, $description, $link);
                
                while($stmt->fetch()){ 
                    echo "<h2> <a href=".htmlspecialchars($link).">".htmlspecialchars($title)."</a> </h3>";
                    /* Button needed for commenting on a story */
                    echo "<h4> ".htmlspecialchars($description)."  <a class='button' href='story_commentsView.php?val=$story_id'> Comments </a></h4>";
                    /*"</h4> <form name='comment' action='addComment_form.php' method='post'> <p> <button type='submit'> Comment </button></p></form>";*/
                }
                $stmt->close();  
                
            ?> 
            
            <!--comments displayed (can be expanded)-->
        </div>
