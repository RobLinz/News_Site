<!-- Display all stories in Favorites -->
<!doctype html>
<html>
    ...
    <body class=homePage>
        ...
        <div id=news>
            <h2 class=ourStory>Favorites: </h2>
            <!--here we will display stories -->
            <?php
                require 'database_news.php';
                session_start();
                          
                $stmt = $mysqli->prepare("SELECT favorite, favorites.story_ID, favorites.user_ID, stories.title, stories.story_description, stories.story_link FROM favorites JOIN stories on (favorites.story_ID = stories.story_ID) where favorites.user_ID=?"); 
                ...
                $stmt->bind_result($favorite, $story_id, $user_id, $title, $description, $link); 
